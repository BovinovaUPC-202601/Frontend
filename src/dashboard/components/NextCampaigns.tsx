...
function relativeLabel(c: Campaign): { text: string; color: string; dotColor: string } {
    const now = dayjs();
    ...
    if (start && end && now.isAfter(start) && now.isBefore(end)) {
        return { text: "En curso", color: "text-[#10A065]", dotColor: "bg-[#10A065]" };
    }
    ...
    if (start && start.isAfter(now)) {
        ...
        if (days === 0) return { text: "Hoy", color: "text-[#10A065]", dotColor: "bg-[#10A065]" };
        if (days === 1) return { text: "Mañana", color: "text-[#3A82B0]", dotColor: "bg-[#3A82B0]" };
        if (days <= 7) return { text: `En ${days} días`, color: "text-[#3A82B0]", dotColor: "bg-[#3A82B0]" };
        if (weeks === 1) return { text: "En 1 semana", color: "text-[#B17A2B]", dotColor: "bg-[#B17A2B]" };
        if (weeks <= 4) return { text: `En ${weeks} semanas`, color: "text-[#B17A2B]", dotColor: "bg-[#B17A2B]" };
        if (months === 1) return { text: "En 1 mes", color: "text-[#B17A2B]", dotColor: "bg-[#B17A2B]" };
        if (months < 12) return { text: `En ${months} meses`, color: "text-[#B17A2B]", dotColor: "bg-[#B17A2B]" };
        return { text: `En ${start.diff(now, "year")} años`, color: "text-[#B17A2B]", dotColor: "bg-[#B17A2B]" };
    }

    if (end && end.isBefore(now)) {
        return { text: "Finalizada", color: "text-[#7E8F82]", dotColor: "bg-[#7E8F82]" };
    }
                ...
                {campaigns.length === 0 ? (
                    ...
                    visibleCampaigns.map(c => {
                        ...
                        return (
                            ...
                            >
                                <div className={`w-2 h-2 rounded-full shrink-0 ${label.dotColor}`} />
                                <div className="flex flex-col flex-1 min-w-0">
                                    <span className="text-[#0E1A12] text-sm font-semibold font-inter truncate">
                                        {c.name}
                                    </span>
                                    <div className="flex items-center gap-2">
                                        <span className={`text-xs font-inter font-medium ${label.color}`}>
                                            {label.text}
                                        </span>
                                ...
                                <ChevronRightIcon className="w-4 h-4 text-[#7E8F82] group-hover:text-[#10A065] transition-colors duration-150 shrink-0" />
                            </div>
                        );
                    })
                )}
