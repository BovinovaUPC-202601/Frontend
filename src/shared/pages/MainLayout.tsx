...
export function MainLayout() {
    ...
    // Load real permissions + plan from the backend on mount so gating survives
    // refresh (the auth store resets on reload while the token persists).
    // Staff get the OWNER's plan from the profile (the subscription endpoint is
    // owner-only and would answer 403), so fetchCurrentPlan runs only for owners.
    useEffect(() => {
        (async () => {
            await fetchPermissions();
            if (!useAuthStore.getState().user.isStaff) fetchCurrentPlan();
        })();
    }, [fetchPermissions, fetchCurrentPlan]);
